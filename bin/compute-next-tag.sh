...
#
# Tags look like `v<Browserless version>-<release>`, which is what this
# repository has always published (v2.37.1-0 ... v2.56.0-0):
#
# - if defaults/main.yml points at a Browserless version that has never been
#   released, the release counter restarts at 0 (`v2.57.0-0`)
# - otherwise the counter is incremented (`v2.57.0-1`), but only if something
...
# commits: every other change to the role - including ones that change what a
# playbook run does - went unreleased until the next version bump happened to
# come along and carry it.
...
# Anchored on `browserless_version:` so that neither the Renovate annotation
# above it nor `browserless_container_image_tag`, which is derived from it, can
# be mistaken for it.
version="$(sed -nE 's|^browserless_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Browserless version from $defaults_path"
	exit 1
fi

# Browserless's own version is carried with a leading `v` (`v2.56.0`), which
# the tags carry too. Stripping it before adding it back keeps the prefix
# right if that convention ever changes.
